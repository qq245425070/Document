...
实际上到现在为止, 它依然是虚拟机运行在Client模式下的默认年轻代收集器;   
...
ParNew收集器除了多线程收集之外, 其他与Serial收集器相比并没有太多创新之处, 但它却是许多运行在Server模式下的虚拟机中首选的年轻代收集器,   
其中有一个与性能无关但很重要的原因是, 除了Serial收集器外, 目前只有它能与CMS收集器配合工作;    


❀ Parallel Scavenge收集器  
Parallel Scavenge收集器是一个年轻代收集器, 它也是使用复制算法的收集器, 又是并行的多线程收集器  
