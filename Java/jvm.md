...
### 判断对象是否存活与引用有关  
垃圾收集算法; 可达性分析算法;  垃圾收集器;  GC Roots;  JVM怎么判断对象是否已死;  finalize;  内存分配与回收策略;  
...
弱引用: 只要一个对象是 gc root 可达, 不存在强引用(对象引用置为null) 和 弱引用(对象引用置为null), 存在软引用可达, 当垃圾收集器工作时,   
...
Scavenge GC  
一般情况下, 新生成的对象在年轻代 Eden 区申请失败时, 就会触发 Scavenge  GC,  
